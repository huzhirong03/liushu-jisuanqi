            ...
            <div class="setting-group" style="background:linear-gradient(135deg,rgba(102,126,234,0.2),rgba(118,75,162,0.2));border:1px solid rgba(102,126,234,0.3);">
                ...
                <div style="margin-top:12px;">
                    ...
                    <div id="ai-model-buttons" style="display:flex;gap:8px;flex-wrap:wrap;">
                        ...
                        <button type="button" class="ai-model-btn" data-provider="google" onclick="switchAIModel('google')">
                            🌐 谷歌
                        </button>
                        <button type="button" class="ai-model-btn" data-provider="qwen" onclick="switchAIModel('qwen')">
                            ☁️ 千问
                        </button>
                        <button type="button" class="ai-model-btn" data-provider="glm" onclick="switchAIModel('glm')">
                            🧠 智谱
                        </button>
                    </div>
...
连码|连码?(\d+(?:[,，]\d+)+)\s*(\d+)|连码"></textarea>
    ...
    <div class="import-modal" id="multi-model-modal" style="display:none;">
        <div class="import-modal-content" style="max-width:95%;width:500px;max-height:90vh;overflow:hidden;display:flex;flex-direction:column;">
            ...
            <div id="model-tabs" style="display:flex;gap:0;margin-bottom:0;border-bottom:1px solid rgba(255,255,255,0.1);">
                ...
                <button class="model-tab" data-model="google" onclick="switchModelTab('google')">
                    <div class="model-tab-name">Gemini</div>
                    <div class="model-tab-time" id="tab-time-google">等待中...</div>
                </button>
                <button class="model-tab" data-model="glm" onclick="switchModelTab('glm')">
                    <div class="model-tab-name">智谱</div>
                    <div class="model-tab-time" id="tab-time-glm">等待中...</div>
                </button>
            </div>
    ...
    const AI_PROVIDERS = {
        ...
        google: {
            ...
            region: 'direct',  // 兔子API第三方中转，国内直连无需代理
        ...
        glm: {
            ...
            model: 'glm-4-flash',
            region: 'cn',
            apiKey: '441ca23345664b47bb0ba8bfcb8856f5.vGvRMBnk6pSJnB9q'
        }
    };
    ...
    let multiModelResults = {
        ...
        google: { status: 'pending', time: 0, raw: '', parsed: [], error: '' },
        glm: { status: 'pending', time: 0, raw: '', parsed: [], error: '' }
    };
    ...
    async function runMultiModelTest() {
        ...
        multiModelResults = {
            ...
            google: { status: 'running', time: 0, raw: '', parsed: [], error: '' },
            glm: { status: 'running', time: 0, raw: '', parsed: [], error: '' }
        };
        
        // 更新UI显示"运行中"
        ['deepseek', 'qwen', 'google', 'glm'].forEach(model => {
        ...
        // 构建提示词（复用现有逻辑）
        let userInput = input;
        if (typeof normalizeChineseText === 'function') {
            userInput = normalizeChineseText(input);
        }
        
        const prompt = `你是下注类型分类器。任务：识别每条下注属于什么玩法类型。

【你只需要做一件事】判断玩法类型，不需要提取数字！

【玩法类型】共23种：
1. 特码 - 含"特"字或纯数字下注(01-49)
2. 生肖特码 - "买X各Y"格式，买某生肖的所有号码做特码
3. 生肖 - 单个生肖下注如"猪10""龙x10"
4. 波色 - 含"红/蓝/绿"+"波"
5. 半波 - 波色+大小/单双
6. 平特一肖 - 含"平特"或"一友"
7. 平特二连肖 - 含"二连肖"或"二友"
...
【输出格式】严格按此JSON格式：
{"items":[{"type":"类型","raw":"原文"}]}`;
        
        // 并行调用所有模型（4个）
        const modelsToTest = ['deepseek', 'qwen', 'google', 'glm'];
    ...
    function displayModelResult(model) {
        ...
        if (result.status === 'running') {
            ...
            parsedEl.innerHTML = result.parsed.map(bet => 
                `${bet.type} → ${bet.numbers?.join(',') || bet.target || ''} × ${bet.amount}元`
            ).join('<br>');
        } else {
    ...
    async function callAIAPI(prompt, provider, apiKey, retryCount = 0) {
        ...
        const targetUrl = useProxy ? proxyUrl : config.url;
    ...
    async function parseWithAI(userInput) {
        ...
        // ╔════════════════════════════════════════════════════════════════╗
        // ║          【AI提示词开始】发送给大模型的提示词                    ║
        // ║  AI只做语义识别（判断玩法类型），数值提取由程序完成              ║
        // ║  这样更高效：AI运行快、程序计算准、稳定性高                      ║
        // ╚════════════════════════════════════════════════════════════════╝
        const prompt = `你是下注类型分类器。任务：识别每条下注属于什么玩法类型。
...
【玩法类型】共23种：
1. 特码 - 含"特"字或纯数字下注(01-49)
2. 生肖特码 - "买X各Y"格式，买某生肖的所有号码做特码，每个号码各Y元！！重要！！
3. 生肖 - 单个生肖下注如"猪10""龙x10"（不含"买"和"各"）
4. 波色 - 含"红/蓝/绿"+"波"
5. 半波 - 波色+大小/单双，如"红单""蓝双""绿大"
6. 平特一肖 - 含"平特"或"一友"，买生肖
7. 平特二连肖 - 含"二连肖"或"二友"
8. 平特三连肖 - 含"三连肖"或"三友"
9. 平特四连肖 - 含"四连肖"
10. 平特五连肖 - 含"五连肖"
11. 平特尾数 - 含"平特尾"或"平尾"
12. 六肖 - 含"六肖"，买6个生肖
13. 单平 - 含"单平"或"平码"（除特码外6个正码）
14. 大小单双 - 单独的"大/小/单/双"
15. 二中二 - 含"二中二"
16. 三中三 - 含"三中三"
17. 不中 - 含"X不中"如"五不中""十二不中"
18. 合肖 - 含"合肖"或"X肖"（非六肖）
19. 连码 - 含"二全中""三全中""二中特""三中特""特串"
20. 正肖 - 含"正肖"
21. 总和 - 含"总和大/小/单/双"
22. 五行 - 含"金/木/水/火/土"
23. 家野 - 含"家禽""野兽"
...
【关键区分】
- "买猪各10" → 生肖特码（买猪的号码7,19,31,43各10元=40元，赔率43）
- "猪10" 或 "猪x10" → 生肖（买猪生肖10元，赔率12）
...
【示例】
输入: "买猪各10 猪x20 五不中01,02,03,04,05各100"
输出: {"items":[{"type":"生肖特码","raw":"买猪各10"},{"type":"生肖","raw":"猪x20"},{"type":"不中","raw":"五不中01,02,03,04,05各100"}]}
